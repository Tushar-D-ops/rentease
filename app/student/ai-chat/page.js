...
export default function AIChatPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col h-[calc(100vh-80px)]">
      <div className="mb-6">
        <h1 className="font-syne font-bold text-3xl text-white mb-1">AI Assistant</h1>
        <p className="text-[#7b82a8]">Powered by Google Gemini · Free tier</p>
      </div>

      <div className="flex-1 bg-[#111527] border border-white/5 rounded-2xl flex flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-3 ${msg.role==='user'?'flex-row-reverse':''}`}>
              ...
              <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${msg.role==='user'?'bg-brand-500 text-white rounded-tr-none':'bg-[#1a2035] text-white/90 rounded-tl-none'}`}>
                {msg.content}
              </div>
            </div>
          ))}
          ...
          <div className="px-4 pb-2 flex flex-wrap gap-2">
            {SUGGESTIONS.map((q) => (
              <button key={q} onClick={() => sendMessage(q)}
                className="text-xs bg-white/5 hover:bg-brand-500/10 hover:text-brand-400 text-[#7b82a8] px-3 py-1.5 rounded-full transition-colors border border-white/5 hover:border-brand-500/20">
                {q}
              </button>
            ))}
          </div>

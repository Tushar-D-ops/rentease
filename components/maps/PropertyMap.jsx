...
export default function PropertyMap({ properties = [], collegeLat, collegeLng, height = '400px' }) {
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      if (collegeLat && collegeLng) {
        ...
        L.marker([collegeLat, collegeLng], { icon:collegeIcon }).addTo(map).bindPopup('<b>Your College</b>')
      }
      ...
      properties.forEach((prop) => {
        ...
        L.marker([prop.lat, prop.lng], { icon }).addTo(map).bindPopup(`
  ...
  return <div ref={mapRef} style={{ height, width:'100%', borderRadius:'16px', overflow:'hidden' }} className="border border-white/5" />
}